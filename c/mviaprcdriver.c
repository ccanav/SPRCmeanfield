...
double phi,tau_2,arg,f;
double x,xend,h;
FILE *fp1,*fp2,*fp3,*fp4,*fp5,*fp6,*pi,*ip,*tc;
int i,j;
...
gsyn2=(1.0/(1-exp(-T/TAU_FALL))) -(1.0/(1.0-exp(-T/TAU_TWO)));
gsyn2=f*gsyn2*G_SYN;
scan_(state);
...
//{for(counter=56;counter<57;counter++)
 {x = 0.0;
gsyn1 = 0.0;
  h=1e-5;
 xend=  (counter*period[0])/(1.0*INCREMENT) ;
  ts =  (counter*period[0])/(1.0*INCREMENT) ;
