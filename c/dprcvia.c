...
double *xp;
{
extern double  forcingt,ts,gsyn1,gsyn2,current[C]; 
extern int pflag;
int el;
double time_;
...
if(pflag&&*xp>ts-forcingt) gsyn1=f*G_SYN*((exp((ts-forcingt-*xp)/TAU_FALL)-1.0)/(1.0 - exp(-forcingt/TAU_FALL))-(exp((ts-forcingt-*xp)/tau_2)-1.0)/(1.0-exp(-forcingt/tau_2)));
else gsyn1=0.0;
...
if(*xp < forcingt) gsyn1=f*G_SYN*((exp((ts-*xp)/TAU_FALL)-1.0)/(1.0 - exp(-forcingt/TAU_FALL))-(exp((ts-*xp)/tau_2)-1.0)/(1.0-exp(-forcingt/tau_2)));
else gsyn1=0.0;
