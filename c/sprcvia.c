...
double *xp;
{
extern double  ts,gsyn1,gsyn2,forcing,current[C]; 
extern int pflag;
int el;
double time_;
...
if(pflag&&*xp>ts-forcing) gsyn1=f*G_SYN*((exp((ts-forcing-*xp)/TAU_FALL)-1.0)/(1.0 - exp(-forcing/TAU_FALL))-(exp((ts-forcing-*xp)/tau_2)-1.0)/(1.0-exp(-forcing/tau_2)));
else gsyn1=0.0;
...
if(*xp < forcing) gsyn1=f*G_SYN*((exp((ts-*xp)/TAU_FALL)-1.0)/(1.0 - exp(-forcing/TAU_FALL))-(exp((ts-*xp)/tau_2)-1.0)/(1.0-exp(-forcing/tau_2)));
else gsyn1=0.0;
