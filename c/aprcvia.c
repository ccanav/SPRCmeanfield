...
current[I_S2] =  G_SYN2*Y[V1] ;
current[I_S3] =  Y[G1]*(Y[V1] - E_SYN);
F[V1] = (I_STIM-current[I_S2] - current[I_NA1] - current[I_K1] - current[I_K3] - current[I_L1] - current[I_S1] -current[I_S3])/CM;
...
F[X1]=-Y[X1]/TAU_TWO;
F[G1]= (pow(TAU_TWO/TAU_FALL, TAU_FALL/(TAU_TWO-TAU_FALL))*Y[X1] -Y[G1])/TAU_FALL;
...
{
extern int gflag;
extern double ts, gsyn1,current[C],gsyn3; 
int el;
double time_;
double theta,k1,sigma1,k2,sigma2;
double f,arg,tau_2;
