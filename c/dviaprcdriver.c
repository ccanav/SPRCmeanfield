...
double tau_2,arg,f,oldpf=0.0,oldpn=0.0;
double x,xend,h;
FILE *fp1,*fp2,*fp3,*fp4,*fp5,*fp6,*pi,*ip;
int i,j;
fp1 = fopen("pfpn.data","w");
...
//if (!PRINT) {for(counter=10;counter<11;counter++)
{
...
// fprintf(fp1,"%f %f \n",forcingt, period[0]*(1.0+f1));
 ...
 fprintf(fp1,"%f %f\n",forcingt, period[0]*(1.0+f1));
 if((oldpf<=oldpn)&& (forcingt>=period[0]*(1.0+f1))) printf("%f\n", oldpn);
 oldpf=forcingt;
 oldpn=period[0]*(1.0+f1);
